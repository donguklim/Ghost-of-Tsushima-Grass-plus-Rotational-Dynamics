...
The bars form a multi-body system with rigid bodies.
Articulated Body Algorithm (ABA) is used for simulating the angular acceleration.

In addition, there is a limit on the total angular displacement in each pivot, and approximate ground collision is added.


### Articulated Body Algorithm(ABA)

The external spatial forces (which are mostly torque in this case) are calculated by integrating torque along each bar.

- Bar1 receives torque from wind force, air friction due to angular velocity, and restoration torque.
- Bar2 is also similar, but Bar2 receives additional air friction due to Bar1's angular velocity.

The bars form a multi-body system with rigid bodies. Unlike the single bar case, multi-body dynamics have forces transmitted between each body,
and angular acceleration cannot be calculated by simply dividing the torque by moment of inertia.

Advanced multi-body dynamics algorithms are required.
...
This is an inaccurate algorithm that I made before I learned about ABA.
I am leaving this section as a record, and allow to use this algorithm by checking the 'Use Faster Algorithm' check box.

However, I am not really sure if this algorithm is significnatly fast.
It does less number of math operations than ABA in HLSL code, but there isn't any change in time complexity.

Thanks for Cloude AI who didn't tell me about Multi-body Dynamics, when I first asked what I should research.
I have spent two weeks making this algorithm without a firm confidence and with some agony.

Cloude told me about Multi-body Dynamics when I asked him/she a similar question one or two months later. It sure had some update.
...
This part is not used in current version.

The payback force gives weird motions if grass has nearly straight form.
...
\displaylines{
    \begin{align}
        ...
        \\
        & = -\frac{|\overrightarrow{W} \times \overrightarrow{bar2}|W}{4|\overrightarrow{W}|}
    \end{align}
}
```
...
- [Rigid Body Dynamics Algorithms](https://link.springer.com/book/10.1007/978-1-4899-7560-7) - Roy Featherstone's book
